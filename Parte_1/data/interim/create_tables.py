...
def concatenate_txt_to_csv():
    """
	Function that gets a directory as input, walks through the directory and concatenates .txt files into a single csv file.

	Parameters (Input defined):
	----------
	dir: string
		directory of raw data to be concatenated into a csv file
	
	Output:
	csv file in the processed data 
	"""
...
def insert_csv():
    ...
    try:
        cur = conn.cursor()
        # create table one by one
        dir = input("Choose the directory to search for .csv files: ")
        while not os.path.isdir(dir):
            print("Directory doesn't exist. Please try again")
            dir = input("Choose the directory to search for .csv files: ")
        cur.execute("""SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'""")
...
def main():
    ...
    """
    option_dict = {1:'Transform .txt files from dinning data into a single .csv file (not required if you already have the crudapp_e_dinning.csv file in the processed folder)', 2: 'Generate random data for the crudApp_a_student table. This will generate a csv file called crudapp_a_studen.csv and saved in the processed folder.',3:'Create all the tables (crudApp_a_student, crudApp_b_grade, crudApp_c_lonely,crudApp_d_stress,crudApp_e_dinning)',4:'Insert the data in the tables.', 5:'Print the plot (distribution of overall GPA per subject/field of study)',6:'This option will 1) create tables, 2) ask for the directory with the csv files to add to the tables and 3) print the plot (essentially doing option 3,4,5 all together).',7:'Exit'}
    print("############### \n ATTENTION: If you already have all the csv files in a single folder the only options you need are 3, 4 and 5. If you want to try to concatenate the .txt files from the dinning folder into a single csv file, use option 1. If you want to generate new random data to populate the table called 'a_table', press 2 \n ###############")
